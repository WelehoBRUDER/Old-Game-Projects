...
function HurtPlayer() {
    ...
    if(enemy[currentEnemy].hp <= 0) {
        ...
        if (enemy[currentEnemy].dead == true && enemy[currentEnemy].dead2 == false) {
            ...
            if(enemy[currentEnemy].id == "boss1" && bosses.Boss1 !== "beaten") {
                bosses.Boss1 = "dead";
            }
            for(let b = 0; b<CurrentQuests.length; b++) {
                if(enemy[currentEnemy].id.startsWith(CurrentQuests[b].Type)) {
...
function StatusCheck() {
    for (let i = 0; i < attacks.length; i++) {
        if ((attacks[i].name == "Burn"|| attacks[i].name == "Poison Wind") && Math.random() < 30 / 100) {
