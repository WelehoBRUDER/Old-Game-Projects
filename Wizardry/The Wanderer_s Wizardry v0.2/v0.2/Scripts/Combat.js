...
function EndScreen(g) {
    if (g == 1) {
        ...
        p.textContent = `You've won the battle! You gain ${player.exp - player.startexp} exp!`;
        ...
        let p = document.createElement('p');
        p.classList.add("BattleText");
        p.textContent = `You've lost the battle. Though you still gain ${Math.floor(player.exp - player.startexp)} exp for the enemies you defeated!`;
        let b = document.createElement('button');
        b.classList.add("BattleEndButton");
        b.textContent = "Continue";
        BE.appendChild(T);
...
function StatusCheck() {
    for (let i = 0; i < attacks.length; i++) {
        if (attacks[i].type == "fire" && Math.random() < 100 / 100) {
