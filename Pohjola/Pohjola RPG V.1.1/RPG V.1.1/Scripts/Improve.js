...
function LoadStats() {
    ...
    document.getElementById('cha').textContent = `CHA: ${PlayerStats.cha}`;
    ...
    document.getElementById('cha').addEventListener('mouseover', () => StatInfo(6));
    document.getElementById('cha').addEventListener('mouseout', () => StatInfo(0));
...
function StatInfo(a) {
    if (a == 0) {
        ...
        StatsD.textContent = 'Mind improves your magical efficiency by 5% per level and increases mana by 25.';
    } else if (a == 5) {
        StatsD.textContent = 'Agility improves speed slighty every 3 levels. Speed determines how fast you act in combat.';
    } else if (a == 6) {
        StatsD.textContent = 'Charisma reduces shop prices and improves character relations.';
    } else if (a == 7) {
...
function ImprovChar(g) {
    if (PlayerStats.SF >= PlayerStats.nextlvl) {
        if (g == 1) {
...
function PerkDescription(j) {
    if (j == 0) {
        ...
        document.getElementById('PerkDesc').textContent = `Deadly Blade grants +7 to base damage and +50 HP. Cost: ${PlayerWarPerks.C4} SF.`;
    } else if (j == 5) {
        ...
        document.getElementById('PerkDesc').textContent = `Blademaster grants +13 to base damage. Cost: ${PlayerWarPerks.C7} SF.`;
    }
}
...
function BuyItem(x) {
    if (x == 1) {
        ...
        if (PlayerStats.Gold >= StorePrices.wp5 && PlayerEquip.bought9 == 0) {
