...
function UseSkill(e) {
    if (playerTurn) {
        if (e == 1 && PlayerAbilities.SunderCD == 0) {
            ...
            EnemyStats.dmgT = (PlayerStats.dmg * 1.5) * PlayerStats.truwis - EnemyStats.def;
            EnemyStats.dmgT = Math.floor(EnemyStats.dmgT);
            EnemyStats.health -= EnemyStats.dmgT;
            ShowCooldowns();
...
function CastMagic(f) {
    if (f == 1 && playerTurn) {
        ...
        if (PlayerStats.mana >= 75) {
            EnemyStats.dmgT = 50 * PlayerStats.truwis - EnemyStats.def;
            EnemyStats.dmgT = Math.floor(EnemyStats.dmgT);
            if (EnemyStats.dmgT <= 0) {
                EnemyStats.dmgT = 1;
            }
...
function ShowSkill(h) {
    ...
    if (h == 0) {
        ...
        t.textContent = 'Spell: Fireball, deals 50 damage, influenced by WIS. MP cost: 75.';
        adds.style.opacity = 1;
        t.style.opacity = 1;
    }
}
