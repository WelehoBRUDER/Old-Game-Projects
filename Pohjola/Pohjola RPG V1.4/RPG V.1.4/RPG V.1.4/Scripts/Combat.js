...
function UseSkill(e) {
    if (playerTurn) {
        if (e == 1 && PlayerAbilities.SunderCD == 0) {
            ...
            if(EnemyStatusEffects.Sundered > 0) {
                EnemyStats.dmgT = PlayerStats.dmg * 1.5;
                ...
                EnemyStats.dmgT = Math.floor(EnemyStats.dmgT - EnemyStats.def/2);
            } else {
                EnemyStats.dmgT = PlayerStats.dmg * 1.5;
                if(PlayerStats.truwis > PlayerStats.trustr) {
                    EnemyStats.dmgT = EnemyStats.dmgT*PlayerStats.truwis;
                ...
                EnemyStats.dmgT = Math.floor(EnemyStats.dmgT - EnemyStats.def);
            }
            if(EnemyStats.dmgT <= 0) {
                EnemyStats.dmgT = 1;
            }
...
function CastMagic(f) {
    if (f == 1 && playerTurn) {
        ...
        if (PlayerStats.mana >= 75) {
            if (EnemyStatusEffects.Sundered > 0) {
                EnemyStats.dmgT = PlayerSpells.FireDMG * PlayerStats.truwis - EnemyStats.def / 2;
                EnemyStats.dmgT = Math.floor(EnemyStats.dmgT);
                if (EnemyStats.dmgT <= 0) {
                    EnemyStats.dmgT = 1;
                }
                ...
                EnemyStats.dmgT = PlayerSpells.FireDMG  * PlayerStats.truwis - EnemyStats.def;
                EnemyStats.dmgT = Math.floor(EnemyStats.dmgT);
                if (EnemyStats.dmgT <= 0) {
                    EnemyStats.dmgT = 1;
                }
        ...
        if (PlayerStats.mana >= 100) {
            if (EnemyStatusEffects.Sundered > 0) {
                EnemyStats.dmgT = PlayerSpells.MeteorDMG * PlayerStats.truwis - EnemyStats.def / 2;
                EnemyStats.dmgT = Math.floor(EnemyStats.dmgT);
                if (EnemyStats.dmgT <= 0) {
                    EnemyStats.dmgT = 1;
                }
                ...
                EnemyStats.dmgT = PlayerSpells.MeteorDMG * PlayerStats.truwis - EnemyStats.def;
                EnemyStats.dmgT = Math.floor(EnemyStats.dmgT);
                if (EnemyStats.dmgT <= 0) {
                    EnemyStats.dmgT = 1;
                }
...
function ShowSkill(h) {
    ...
    if (h == 0) {
        ...
        t.textContent = 'Skill: Focused Dash, Deal 150% dmg, influenced further by WIS. Cooldown: 2 turns. MP cost: 100.';
        adds.style.opacity = 1;
        t.style.opacity = 1;
    }
    else if (h == 6) {
...
function Boss2Attack() {
    ...
    if (EnemyStats.health < 2000 && BossAbilities.Heal < 2) {
        ...
        BossAbilities.HardenCD = 4;
        BossAbilities.Harden = 3;
        EnemyStats.def = EnemyStats.def * 1.5;
